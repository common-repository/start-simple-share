...
function msbd_share_buttons_markup($isShortcode=false) {
    global $post, $sssPO;
    ...
    if( (!$isShortcode && is_single() && in_array($post_type, $arrSettings['sssp_post_types'])) || 
        (!$isShortcode && $post_type=="page" && is_page() && in_array($post_type, $arrSettings['sssp_post_types'])) || 
        (!$isShortcode && $post_type=="post" && $arrSettings['sssp_show_with_post_excerpt']=="yes") ||
        ($isShortcode && (is_single() || is_page())) ) {
        ...
        // ssba div
        $htmlShareButtons = '<!-- Start Simple Share microsolutionsbd.com --><div class="sssp-wrap type-'.$post_type.'">';
...
function sssp_pinterest($arrSettings, $urlCurrentPage, $strPageTitle, $booShowShareCount) {
    ...
    if($arrSettings['sssp_pinterest_featured'] == 'yes') {
        ...
        if(isset($arrSettings['post_id']) && has_post_thumbnail($arrSettings['post_id'])) { // if this post has a featured image
